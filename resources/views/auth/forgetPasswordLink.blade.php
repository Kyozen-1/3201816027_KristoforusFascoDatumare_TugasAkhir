...
<html lang="en">
	...
	<head>
		<meta charset="utf-8" />
		<title>Reset Password | Pontianak Covid-19</title>
					...
					<div class="d-flex flex-row-fluid flex-column justify-content-between">
						...
						<div class="d-flex flex-column-fluid flex-column flex-center mt-5 mt-lg-0">
							<a href="{{route('home')}}" class="mb-15 text-center">
								<img src="{{ asset('metronic/assets/media/logos/logo-letter-1.png') }}" class="max-h-75px" alt="" />
							</a>
							<!--begin::Signin-->
							<div class="login-form login-signin">
								...
								<!--begin::Form-->
								<form action="{{route('reset.password.post')}}" method="POST" class="form" novalidate="novalidate">
									@csrf
									<input type="hidden" name="token" value="{{$token}}">
									<div class="form-group py-3 m-0">
										<input id="email_address" type="email" class="form-control h-auto border-0 px-0 placeholder-dark-75" name="email" placeholder="Email Address" autocomplete="off" required autofocus>
                                        @if ($errors->has('email'))
                                            <span class="text-danger">{{ $errors->first('email') }}</span>
                                        @endif
									</div>
									<div class="form-group py-3 border-top m-0">
										<input id="password" type="password" class="form-control h-auto border-0 px-0 placeholder-dark-75" name="password" placeholder="Password" autocomplete="off" required autofocus>
                                        @if ($errors->has('password'))
                                            <span class="text-danger">{{ $errors->first('password') }}</span>
                                        @endif
									</div>
                                    <div class="form-group py-3 border-top m-0">
										<input id="password-confirm" type="password" class="form-control h-auto border-0 px-0 placeholder-dark-75" name="password_confirmation" placeholder="Confirm Password" autocomplete="off" required autofocus>
